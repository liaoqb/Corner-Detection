// <Copyright liaoqb>  [Copyright 2015.04.12]
...
struct PairPoint {
  ...
  PairPoint(Point pt1, Point pt2) : pt1(pt1), pt2(pt2) {}
};
...
void sortPoints(vector<Point>& points) {
  ...
  sort(points.begin(), points.end(), [&](Point a, Point b) {
	return (pow(a.x - temp.x, 2) + pow(a.y - temp.y, 2)) <
	  (pow(b.x - temp.x, 2) + pow(b.y - temp.y, 2));
  });
  ...
  for (char i = 'a'; i <= 'p'; ++i) {
	...
	if (check) {
	  ...
	  for (int x = 0; x < points.size(); ++x) {
		line(canny, points[x], points[x] + Point(30, 30), Scalar(255, 0, 0), 30);
	  }
	  */
	  ...
	  try {
		imwrite(result, transform, compression);
